...
namespace Ecommerce.Web.Repositories
{
    public class BasketRepository : IBasketRepository
    {
        ...
        /// </summary>
        /// <returns></returns>
        public IList<BasketItemViewModel> GetBasketItem(string userId)
        {
        ...
        /// <returns></returns>
        public BasketItemViewModel AddItemintoBasket(int productId, int quantity, string userId)
        {
            ...
            string apiResponse;
            //apiResponse = _iGetRequestManager.SendRequest(_iGetApiUrls.GetProductByIdApiUrl(productId), "", "", false, HttpRequestContentType.ApplicationJson.GetDescription(), null);
            //if (apiResponse == null)
            //    return null;
            //GenericResponseRepository<Product> responsePro = new GenericResponseRepository<Product>();
            //Product product = responsePro.Convert(apiResponse);

            //basketItem.Product = product;
            //basketItem.Id = 10;
            string jsonsContent = JsonConvert.SerializeObject(basketItem);
            string contenttypedesc = HttpRequestContentType.ApplicationJson.GetDescription();
            apiResponse = _iPostRequestManager.SendRequest(_iGetApiUrls.PostBasketItem, jsonsContent, "", "", false, contenttypedesc, null);
            if (apiResponse == null)
                return null;
        ...
        /// </summary>
        /// <returns></returns>
        public IList<BasketItemViewModel> DeleteAllBasketItems(string userId)
        {
        ...
        public IList<BasketItemViewModel> Checkout(string userid)
        {
            string apiResponse = _iGetRequestManager.SendRequest(_iGetApiUrls.Checkout(userid), "", "", false, HttpRequestContentType.ApplicationJson.GetDescription(), null);
            if (apiResponse == null)
                return null;
